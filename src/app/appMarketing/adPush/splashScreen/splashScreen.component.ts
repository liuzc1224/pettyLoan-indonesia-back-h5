...
import {SplashScreenService} from '../../../service/appMarketing/splashScreen.service';
...
export class SplashScreenComponent implements OnInit {
  ...
  languagePack: Object;
  splashScreenjump : Array<Object>;
  splashScreenState : Array<Object>;
  tableData: TableData;
  validForm : FormGroup;
  ...
  splashScreenPushWay : Array<Object>;
  ...
  getLanguage() {
    this.translateSer.stream(['appMarketing.splashScreen', 'common'])
      .subscribe(
        data => {
          this.languagePack = {
            common: data['common'],
            data: data['appMarketing.splashScreen'],
          };
          this.splashScreenjump=this.languagePack["data"]["jump"];
          this.splashScreenState=this.languagePack["data"]["state"];
          this.splashScreenPushWay=this.languagePack["data"]["pushWay"];
        }
      );
    this.initialTable();
  };
    ...
    this.tableData = {
      ...
      tableTitle : [
        ...
        },
        {
          ...
          filter:(item)=> {
            let name=this.splashScreenState.filter(v=>{
  ...
  deleteOk() {
    ...
    let data = {
      id:this.sgo.get("splashScreenId").id
    };
    this.service.deleteSplashScreen(data)
      .pipe(
        filter( (res : Response) => {
  ...
  handleOk(){
    ...
    if(mode['id']){
      ...
      this.addsplashScreen(data);
    }
  }
  updatesplashScreen(data){
    this.service.updateSplashScreen(data)
      .pipe(
        filter( (res : Response) => {
          this.isOkLoading = false;
          if(res.success === false){
  ...
  addsplashScreen(data){
    this.service.addSplashScreen(data)
      .pipe(
        filter( (res : Response) => {
          this.isOkLoading = false;
          if(res.success === false){
    ...
    this.service.getPushWay()
      ...
      .subscribe(
        ( res : Response ) => {
          ...
          let $pushWay = this.splashScreenPushWay.filter(item=>{
